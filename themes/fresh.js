...
window.styleThemes.fresh = {
    ...
    textColor: '#3D4F3D',
    metaColor: '#16A34A',
    // 主题色（跟随用户选择的 c.accent，但保留 UI 设计的绿色配色作为参考）
    green: '#22C55E',
    greenDeep: '#15803D',
    greenLight: '#86EFAC',
    greenSoft: '#DCFCE7',
    ...
    h2Style(c, s, sp, t) {
        // 主题色软底圆角卡片 + 主题色深字（按 UI）
        return `margin:${sp.h2MarginTop} 0 ${sp.h2MarginBottom};padding:12px 16px;line-height:1.5;font-size:${s.h2Size};font-weight:800;color:${c.accentDark};background:${c.accentSoft};border-left:4px solid ${c.accent};border-radius:0 10px 10px 0;letter-spacing:0.5px;`;
    },
    h2Decor(c) {
        // 标题下方圆点装饰（按 UI）
        return `<section style="display:flex;align-items:center;margin-top:8px;"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${c.accent};margin-right:10px;"><span leaf=""><br></span></span><span style="font-size:11px;color:${c.accentDark};letter-spacing:2px;font-weight:600;"><span leaf="">SECTION 01</span></span></section>`;
    },
    blockquoteStyle(c) {
        // 主题色软底圆角卡片 + 左主题色边（按 UI）
    ...
    // 尾部名片（清新绿卡片）—— 严格按 UI 设计结构
    introCardHTML(data, ctx) {
        const sp = ctx.sp, s = ctx.s, c = ctx.c;
        const font = ctx.font;
        // 卡片背景用主题色浅色渐变（按 UI 设计）
        const cardStyle = `margin:${sp.pMargin} 0 8px;padding:32px 22px 26px;background:linear-gradient(135deg,${c.accentSoft} 0%,${c.accentBorder} 100%);border-radius:16px;box-shadow:0 6px 20px -6px rgba(34,197,94,0.2);text-align:center;font-family:${font};`;
        // 圆头像：主题色背景 + 白 T（按 UI）
        const avatarStyle = `display:inline-block;width:56px;height:56px;border-radius:50%;background:${c.accent};margin:0 0 14px;line-height:56px;text-align:center;box-shadow:0 6px 16px -2px rgba(34,197,94,0.4);`;
        ...
        // 一句话简介（主题色）
        const titleStyle = `font-size:13px;color:${c.accentDark};margin:0 0 16px;letter-spacing:0.5px;`;
        // 三点分隔（主题色双圆点 + 浅色线）
